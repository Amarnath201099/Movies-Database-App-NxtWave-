...
class Upcoming extends Component {
  state = {upcomingMoviesList: [], totalPages: '', isLoading: true}

  componentDidMount() {
    this.getUpcomingMoviesData()
  }

  getUpcomingMoviesData = async (pageNo = 1) => {
    this.setState({isLoading: true})

    const apiKey = '683445102e37dd8e1342a6724dddcea9'
    const apiUrl = `https://api.themoviedb.org/3/movie/upcoming?api_key=${apiKey}&language=en-US&page=${pageNo}`
    ...
    this.setState({
      upcomingMoviesList: updatedFormatData,
  ...
  render() {
    const {upcomingMoviesList, totalPages, isLoading} = this.state
    ...
    return (
      <>
        ...
        {isLoading ? (
          ...
          <MoviesList moviesData={upcomingMoviesList} />
        )}
        <Pagination
